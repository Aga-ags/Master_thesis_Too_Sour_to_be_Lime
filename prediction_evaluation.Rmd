...
make_ph_lime_confusion_matrix <- function(df, model_name) {
  ...
  df <- df %>%
    ...
    mutate(
      ...
      lime_group = ifelse(lime > 0.0001, "lime > 0", ifelse( lime < 0.0001, "lime < 0", "lime = 0") )
    )
...
mse_of_rule_points_negative_incorrect <- function(model_df){
   metrics <- model_df|>
    filter(pH < 6.5)|>
     group_by(model)|>
     summarise(
      RMSE = sqrt(mean((lime - 0)^2)), # assume actual value is 0
      MAE  = mean(abs(lime  - 0))
    )
  return(metrics)
}
...
```{r f_infared_lime_pH_relationship}
...
```{r}
df<- read.csv(file="./tuning_results/example_validation.csv")|>
  pivot_to_pred_model()
df|>  analyze_output(outcome = "lime")
...
In univariate pH: 
Based on metrics - all models are similar to baseline, with dropout having the best performance. 
Based on scatterplot: Inclusion of regularization prevents the model from predicting higher pH when that is adequate. The graph of with dropout and without look similar.
Based on residual scatterplot: Again, we see a strong cut of when reguralization is used. Use of dropout leads to more negative residuals in the lower pH.
Based on histogram comparison: No dropout leads to the closest replication of the distribution in validation data. Use of dropout leads to prediction of more small pH (which is underrepresented in this sample). Also, again, regularization leads to no really high pH - which is in the sample. 

In univariate lime: 
Based on metrics: Inclusion of dropout - slight improvement of RMSE in this sample, reg makes it a little bit worse. MAE is worse with any measure - this indicates that with dropout there is less really big errors, but the average error is larger. The measures make the prediction of zero lime worse, but the non-zero lime gets better. 
Based on scatterplots: All models very strongly underestimate a lot of the higher lime values.
Based on residual scatterplot: All models show the trend of overestimating small lime values and underestimating large ones. 
Based on barplot residuals: More positive residuals than negative.
Based on histogram comparison: Very clearly no dropout leads to a better estimation of the smaller values. 

Multivariate model:
Matrics: Inclusion of any method reduces performance of pH, but the RMSE of lime is improved. Reguralization and dropout lead to a lower RMSE and MAE. 
Based on scatterplot: Dropout lead to a flatter prediction of pH and lime (there is virtually no trend). I suspect that the bad RMSE score of no methods model stems from the bad prediction of the 3 extreme values, because otherwise it looks better than the dropout model. 
Based on histogram comparison: the models with no methods looks better on both. 

Conclusion: no to dropout. 
...
analyze_output(overfit_df, outcome = "lime")
```
The inclusion of zero-mse leads to this gap in prediction around 0.5  - low values are predicted closer to zero. Generally, it does not seem this is particuraly improving the prediction. 
...
```{r evaluate_consistency_lambda}
# Confusion matrix
make_ph_lime_confusion_matrix(df_best_models, model_name = "both_penalty_0_001")  
make_ph_lime_confusion_matrix(df_best_models, model_name = "both_penalty_0_5") 
make_ph_lime_confusion_matrix(df_best_models, model_name = "both_penalty_0_75") 
...
# Evaluation of consistency of best models
