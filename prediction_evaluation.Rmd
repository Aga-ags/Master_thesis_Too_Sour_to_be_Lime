...
# Test
```{r}
df_test <- read.csv(file="./tuning_results/test2.csv")|>
  pivot_to_pred_model()

analyze_output(df_test, outcome = "pH")
analyze_output(df_test, outcome = "lime")
```
For models with no imputation and penalty: the penalty does not improve pH prediction (more pH <6.5 is predicted than its there) and for some lime it makes more zero to be predicted. So, the 0 lime, pH < 6.5 are predicted more often, but there is also a lot more 0 lime predicted when not needed and non zero limed predicted when it should be zero...

For models with MSE + lambda: actually makes the preditions for univariate better, but for some reason introducing the penalty makes the prediction of both lime worse for zero... Maybe: the term is linear while the MSE is squared, so it takes away the importance of the MSE while itself having a smaller penatly for larger diviation?
...
```{r}
histogram_lime_below_65(df_conistency_lin) 
ggsave(file = "../Figures/lin_penalty_hist_consitency.pdf")
```


```{r plots_for_report, fig.width = 8, fig.height=8}

a <- plot_compare_output_distributions(lambda_df_lin, outcome = "pH")
b <- plot_compare_output_distributions(lambda_df_lin, outcome = "lime")
...
ggsave(file = "../Figures/lin_penalty_hist_pH.pdf")
```
...
```
Best prediction: 0.01, actually better than no penalty. And 100% consitent. 
...
best_models$model <-  fct_recode(best_models$model,  "Univariate pH" = "pH_no_lime_imputation_from_3_5_classes", "Univariate Lime" = "lime_no_lime_imputation_from_3_5_classes",  "Multivariate" = "both_no_lime_imputation_from_3_5_classes", "Informed Multivariate" = "both_no_3_5_classes_penatly_01")
...
df_best_models <- extract_best_model("test_predictions_best_models", best_model_name = "both_no_3_5_classes_penatly_01", rename_to = "Informed Multivariate",df_best_models)
