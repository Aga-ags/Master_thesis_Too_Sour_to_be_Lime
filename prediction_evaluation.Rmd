...
The inclusion of non-overfitting measures improves the accuracy of pH prediction for the univariate and multivariate model, but makes the accuracy of lime models worse. But in reality it just means pH is being predicted over a smaller range for dropout and reg. Additionally, the history plots show worse or not improved validation error. Therefore, none of these are implemented. 
...
analyze_output(lambda_df, outcome = "pH")
```
Best perfomance: lanbda 0.001 (no real change in graphs), then, 0.5 - we can see a split in observed vs predicted 
...
best_models$model <-  revalue(best_models$model, c("pH_no_lime_imputation_from_3_5_classes" = "Univariate pH", "lime_no_lime_imputation_from_3_5_classes" = "Univariate lime", "both_no_lime_imputation_from_3_5_classes" = "Multivariate", "both_no_3_5_classes_penatly_05_correction1" = "Informed Multivariate"))
  ...
  df_long %>%
    ...
    summarise(
      ...
      `MSE zero values` = {
        y_true <- pull(cur_data(), !!observed_col)
        y_pred <- pull(cur_data(), value)
        if (sum(y_true + 0 < 0.0001, na.rm = TRUE) > 0) { # Zero values are not 0, because lime is standardized. Equivalent value of - 0.5062935 is used, however, due to rounding errors a equality is substituted to < with 0.0001 tolarance
          mean((y_pred[y_true + 0 < 0.0001] - y_true[y_true + 0 < 0.0001])^2, na.rm = TRUE)
        } else {
          0
        }
      },
      ...
      `MSE non-zero values` = {
        y_true <- pull(cur_data(), !!observed_col)
        y_pred <- pull(cur_data(), value)
        if (sum(y_true > (0 + 0.0001), na.rm = TRUE) > 0) { # similar situation with standarization and rounding error
          mean((y_pred[y_true > (0 + 0.0001)] - y_true[y_true > (0 + 0.0001)])^2, na.rm = TRUE)
        } else {
          0
        }
...
```{r plots_for_poster_ 1, fig.width = 8, fig.height=3}
plot_observed_predicted <-function(df_long, outcome){
  plot <- df_long |>
    filter(predicted == outcome)|>
    ggplot(aes(x = !!sym(outcome), y = value)) +
...
plot_observed_predicted(best_models, outcome = "pH")
ggsave(file = "../Figures/hist_pH_poster.png")
plot_observed_predicted(best_models, outcome = "lime")
ggsave(file = "../Figures/hist_lime_poster.png")
```
```{r}
# Prepare parameters for de-scaling pH and lime content
scaling_parameters <- read.csv(file= "scaling_parameters.csv")
...
df_best_models <- extract_best_model("test_predictions_best_models", best_model_name = "both_no_3_5_classes_penatly_05_correction1", rename_to = "Informed Multivariate",df_best_models)
...
```{r f_histogram_consistency, fig.width = 7, fig.height=3}
...
```{r f_infared_lime_pH_relationship, fig.width = 7, fig.height=3}
...
```{r}
histogram_lime_below_65(df_best_models)
ggsave(file = "../Figures/hist_below_65_poster.png")
predicted_against_predicted(df_best_models)
ggsave(file = "../Figures/relationship_pH_lime_poster.png")
```
