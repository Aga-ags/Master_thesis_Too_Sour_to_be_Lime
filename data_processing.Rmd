...
# distribution of log lime
df_swiss|>
  filter(lime_site %in% c("3", "4", "5"))|>
  ggplot(aes(x = log(lime+1)))+
  geom_histogram()+
  facet_wrap(vars(lime_site))
...
```
```{r}
# normality tests for default and log transformed lime:
perform_normality_test <- function(lime_class, log_transform){
  lime_values <- df_swiss |>
    filter(lime_site == lime_class & !is.na(lime)) |>
    mutate(lime = if (log_transform) log(lime + 1) else lime) |>
    pull(lime)
  shapiro.test(lime_values)
  }

# H0: data is normally distributed 

perform_normality_test(lime_class = 3, log_transform = FALSE)
perform_normality_test(lime_class = 4, log_transform = FALSE)
perform_normality_test(lime_class = 5, log_transform = FALSE)

perform_normality_test(lime_class = 3, log_transform = TRUE)
perform_normality_test(lime_class = 4, log_transform = TRUE)
perform_normality_test(lime_class = 5, log_transform = TRUE)
```




```{r estimate_normal_dist}
# Normal distributions parameters of classes 3,4 and 5. 
df_norm_estimate <- df_swiss|>
  filter(lime_site %in% c("3", "4", "5") & ! is.na(lime))|>
  group_by(lime_site)|>
  summarise(mean_lime = mean(lime), log_mean_lime = mean(log(lime+ 0.01)), var_lime = var(lime), log_sd_lime = sd((log(lime+1))))
...
df_soil_selected_pH_conv <- df_soil_selected_pH_conv|>
  merge(y= df_norm_estimate, by="lime_site", all.x=TRUE)|>
  mutate(
    ...
    lime = 
           # is lime na: 
           ifelse(!is.na(lime), lime,
                  # is pH below 6.5:
                  ifelse (pH_CaCl2_imputed < 6.5, 0, 
                          # is lime_site observed
                          ifelse(is.na(lime_site), lime, 
                                       # is lime class less than 3
                                       ifelse(lime_site < 3, 0,
                                              # draw from random distribution of class 3:
                                              rlnorm(n(), meanlog = log_mean_lime, sdlog = log_sd_lime)-1)))))
...
df_soil_selected_pH_conv <- df_soil_selected_pH_conv%>%
  filter(is.na(pH_CaCl2_imputed) == FALSE)|>
...
processed_data <- processed_data |>
  ...
  select("pH", "pH_origin", "depth", "x", "y", "lime", "lime_imputed", "recording_year", everything())
