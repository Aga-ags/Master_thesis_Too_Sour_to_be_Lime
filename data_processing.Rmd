...
df_soil_selected <- df_soil_arable |>
  dplyr::select(c("id", "x", "y", "horizon_number", "recording_year",  "depth_from_cm", "depth_to_cm", "survey_type") | contains("pH"))
...
md.pattern(df_soil_selected_pH_conv, rotate.names = TRUE)
```
...
df_soil_selected_pH_conv <- df_soil_selected_pH_conv%>%
  dplyr::select(!c("pH-H2O", "pH-CaCl2", "pH_brightness", "horizon_number"))
```
