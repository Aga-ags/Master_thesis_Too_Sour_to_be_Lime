...
# Processing of rasters and vectors
library(terra)
library(sf)

# Other
library(obliquer) # rotated coordinate axis
...
```{r}
# Identify sites with multiple profiles
...
# Check if any multiple profiles also not anonymized
df_soil_org %>%
...
Remove anonymized points since they are too inaccurate: 
```{r}
# Check number of sites before
length(unique(df_soil_org[["id"]]))

# Check number of anonymized  sites
annonymized_points <- df_soil_org|>
  filter(anonymization == 1)
length(unique(annonymized_points[["id"]]))

# Remove anonymized horizons
df_soil <- df_soil_org|>
  filter(anonymization == 0)
```
...
```{r}
# Check distribution of survey and recording dates
...
```
The data spans from 1960 to 2000, with one point later on. Survey and recording data are identical on this scale. 



# Join with arable land use

```{r}
# load arable land raster
...
```{r}
# Convert soil data into a vector 
points_vect_soil <- vect(df_soil, geom = c("x", "y"), crs = crs(areable_land_raster))

# Extract raster value at each soil point
...
# Subset to only arable land points
...
# Number of sites post arable land
length(unique(df_soil_arable[["id"]])) # the number stays the same - the data was already subset to arable land
```
...
```{r}
# Investigating relationship between CaCl2 pH and site pH
...
# Finding how many points have been plotted
df_soil_org|>
  dplyr::select(`pH-CaCl2`, pH_brightness)|>
  filter((!is.na(`pH-CaCl2`)) & (!is.na(`pH_brightness`)))|>
  nrow()

# Quantifying uncertainty of 1 - 1 imputation of site pH into CaCl2:
# Coralation
...
df_soil_with_cov <- df_soil_with_cov|>
  mutate(skfeucht = as.factor(ifelse(is.na(skfeucht), "0", skfeucht)),
         gwleiter = as.factor(ifelse(is.na(gwleiter), "0", gwleiter)),
         feucht_wild =  as.factor(ifelse(is.na(feucht_wild), "0", feucht_wild)), 
          bedgwleiter  = as.factor(ifelse(is.na(bedgwleiter), "0", bedgwleiter)))
df_soil_with_cov
```
...
write.csv(scaling_params, "./data/processed_data/scaling_parameters.csv", row.names = FALSE)
...
# Data spliting 
Made to be independent of previous part
...
st_write(df_sf, "../data/data_with_training_test_split.shp", append=FALSE)
