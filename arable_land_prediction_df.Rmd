...
# make univariate ph and lime together
names(df_pred_10) <- names(df_pred_10) %>%
  str_replace("^pH_no_lime_imputation_from_3_5_classes$", "univariate.pH") %>%
  str_replace("^lime_no_lime_imputation_from_3_5_classes$", "univariate.lime")
...
for (m in model_names) {
  pH_col <- paste0(m, ".pH")
  lime_col <- paste0(m, ".lime")
  new_col <- paste0(m, ".lime_applied")
  
  df_pred_10[[new_col]] <- ifelse(df_pred_10[[pH_col]] < 6.5, df_pred_10[[lime_col]], 0)
}
...
# 1. Convert dataframe to SpatRaster
r <- rast(df_pred_10, type = "xyz", crs = "EPSG:2056" )

out_path <- file.path(paste0("../data/prediction_map_raster.tif"))
